...
global x_axis_left, y_axis_left, x_axis_right, y_axis_right, trigger_left_mapped, trigger_right_mapped, ms_speed, invert_left, invert_right
...
# Initialize Pygame for joystick input (optional, no longer required to exit)
pygame.init()
pygame.joystick.init()

# Check for joystick, but continue if none is found
joystick = None
if pygame.joystick.get_count() > 0:
    joystick = pygame.joystick.Joystick(0)
    joystick.init()
    print("Physical joystick detected.")
else:
    print("No physical joystick detected. Using slider controls.")
...
# Button state tracking to avoid spamming (default to 15 if no joystick)
last_button_states = [False] * (joystick.get_numbuttons() if joystick else 15)
...
def send_joystick_data(dt):
    global x_axis_left, y_axis_left, x_axis_right, y_axis_right, trigger_left_mapped, trigger_right_mapped, last_button_states, virtualloop_enabled, invert_left, invert_right
    if joystick:
        pygame.event.pump()
        ...
        # Use slider values when no joystick is present
        pass

    # Disable left joystick movement when virtualloop is enabled
    if virtualloop_enabled and joystick:
        x_axis_left = 0
...
class ControlApp(App):
    def build(self):
        ...
        # Sliders for motor control
        motor1_slider = Slider(min=-1.0, max=1.0, value=0.0, size_hint=(1, None), height=50)
        def on_motor1_value(instance, value):
            global x_axis_left
            x_axis_left = value
        def reset_motor1_slider(instance, value):
            if not instance.focus:
                instance.value = 0.0
                x_axis_left = 0.0
        motor1_slider.bind(value=on_motor1_value)
        motor1_slider.bind(on_touch_up=reset_motor1_slider)
        layout.add_widget(motor1_slider)

        motor2_slider = Slider(min=-1.0, max=1.0, value=0.0, size_hint=(1, None), height=50)
        def on_motor2_value(instance, value):
            global y_axis_left
            y_axis_left = value
        def reset_motor2_slider(instance, value):
            if not instance.focus:
                instance.value = 0.0
                y_axis_left = 0.0
        motor2_slider.bind(value=on_motor2_value)
        motor2_slider.bind(on_touch_up=reset_motor2_slider)
        layout.add_widget(motor2_slider)

        motor3_slider = Slider(min=-1.0, max=1.0, value=0.0, size_hint=(1, None), height=50)
        def on_motor3_value(instance, value):
            global x_axis_right
            x_axis_right = value
        def reset_motor3_slider(instance, value):
            if not instance.focus:
                instance.value = 0.0
                x_axis_right = 0.0
        motor3_slider.bind(value=on_motor3_value)
        motor3_slider.bind(on_touch_up=reset_motor3_slider)
        layout.add_widget(motor3_slider)

        zoom_slider = Slider(min=-1.0, max=1.0, value=0.0, size_hint=(1, None), height=50)
        def on_zoom_value(instance, value):
            global trigger_left_mapped
            trigger_left_mapped = value
        def reset_zoom_slider(instance, value):
            if not instance.focus:
                instance.value = 0.0
                trigger_left_mapped = 0.0
        zoom_slider.bind(value=on_zoom_value)
        zoom_slider.bind(on_touch_up=reset_zoom_slider)
        layout.add_widget(zoom_slider)
        ...
        buttons = [
            ...
            ("Invert Left Slider", lambda x: toggle_invert_left()),
            ("Invert Right Slider", lambda x: toggle_invert_right()),
            ("Increase msSpeed", lambda x: increase_ms_speed()),
            ("Decrease msSpeed", lambda x: decrease_ms_speed()),
        ]
...
def toggle_invert_left():
    ...
    print(f"Left Slider Inversion: {invert_left}")

def toggle_invert_right():
    global invert_right
    invert_right = not invert_right
    print(f"Right Slider Inversion: {invert_right}")

# Functions to adjust msSpeed
def increase_ms_speed():
    global ms_speed
    ms_speed += 100
    send_udp_message(f"SET_MSSPEED {ms_speed}")
    ms_speed_label.text = f"msSpeed: {ms_speed}"
    save_params_to_json()
    print(f"msSpeed increased to {ms_speed}")

def decrease_ms_speed():
    global ms_speed
    ms_speed = max(100, ms_speed - 100)  # Ensure ms_speed doesn't go below 100
    send_udp_message(f"SET_MSSPEED {ms_speed}")
    ms_speed_label.text = f"msSpeed: {ms_speed}"
    save_params_to_json()
    print(f"msSpeed decreased to {ms_speed}")
