...
# UDP Configuration
UDP_IP = "192.168.1.64"  # Arduino's IP Address
...
def send_joystick_data(dt):
    ...
    for i in range(num_axes):
        ...
        if abs(axis_value) > DEADZONE or i >= 4:  # Show triggers even at rest
            print(f"Axis {i}: {axis_value:.3f} (Mapped: {trigger_left_mapped:.3f} for LT, {trigger_right_mapped:.3f} for RT if i=4,5)")
