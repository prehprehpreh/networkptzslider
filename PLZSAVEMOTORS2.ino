...
void setup() {
    ...
    while (millis() - startTime < CONTROLLER_TIMEOUT) {
        ...
        if (packetSize) {
            ...
            if (positionsReceived && presetAReceived && presetBReceived) break;
        }
    }

    // If controller was found but not all data received, request it
    if (controllerFound && (!positionsReceived || !presetAReceived || !presetBReceived)) {
    ...
    } else if (positionsReceived && presetAReceived && presetBReceived) {
