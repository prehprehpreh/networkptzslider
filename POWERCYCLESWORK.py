...
global initial_setup_complete  # New flag to track initial setup completion
x_axis_right = 0.0
y_axis_right = 0.0
...
invert_left = False
invert_right = False
initial_setup_complete = False  # Initialize flag
...
# File path for saving motor parameters (absolute path to Downloads folder)
    ...
    while True:
        try:
            ...
            if "CURRENT_POS:" in message:
                ...
                # Mark initial setup as complete after receiving the first CURRENT_POS
                ...
                # Reload params directly from file every time to ensure latest data
                try:
                    with open(PARAMS_FILE, "r") as f:
                        params = json.load(f)
                    ...
                    if params:
                        ...
                        # Add safeguard: if position is unexpectedly high (e.g., 10000), log and use last known UI value
                        if motor1_pos > 9000 and int(current_pos1_label.text.split(":")[1].strip()) < 9000:
                            print(f"Warning: Loaded position {motor1_pos} seems invalid, using UI value {current_pos1_label.text.split(':')[1].strip()}")
...
def send_joystick_data(dt):
    ...
    # Removed automatic SEND_CURRENT_POS

# Kivy UI class
class ControlApp(App):
    def build(self):
        global ms_speed, current_pos1_label, current_pos2_label, current_pos3_label, current_pos4_label, preset_a_label, preset_b_label, ms_speed_label
        ...
        buttons = [
            ...
            ("Get Current Position", lambda x: send_udp_message("SEND_CURRENT_POS"))  # New button
        ]
        for text, cmd in buttons:
            layout.add_widget(Button(text=text, on_press=lambda x, c=cmd: c(x)))
        
        # Load parameters from JSON and send to Arduino
        ...
        if params:
            ...
            # Send data to Arduino with delays
            send_udp_message(f"SET_POS {params['current_positions']['motor1']},{params['current_positions']['motor2']},{params['current_positions']['motor3']},{params['current_positions']['motor4']}")
            time.sleep(0.5)  # Increased delay
            send_udp_message(f"SET_PRESET_A {params['preset_a']['motor1']},{params['preset_a']['motor2']},{params['preset_a']['motor3']},{params['preset_a']['motor4']}")
            time.sleep(0.5)  # Increased delay
            send_udp_message(f"SET_PRESET_B {params['preset_b']['motor1']},{params['preset_b']['motor2']},{params['preset_b']['motor3']},{params['preset_b']['motor4']}")
            time.sleep(0.5)  # Increased delay
            send_udp_message(f"SET_MSSPEED {params['ms_speed']}")
        else:
