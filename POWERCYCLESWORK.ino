...
int saved_position1 = 0, saved_position2 = 0, saved_position3 = 0, saved_position4 = 0;
...
const unsigned long updateInterval = 5;  // Update every 5ms (200 Hz) for smoothing
unsigned long previousMillis = 0;
unsigned long lastPresetDebugTime = 0; // For periodic preset debugging
const unsigned long PRESET_DEBUG_INTERVAL = 5000; // Debug presets every 5 seconds

// Flags to track receipt of data
bool positionsReceived = false;
bool presetAReceived = false;
bool presetBReceived = false;
bool initialPositionsSet = false; // Tracks if SET_POS has been received
...
void setup() {
    ...
    // Initialize stepper settings (no position setting yet)
    ...
    // Request positions from Controller immediately
    display.setCursor(0, 20);
    display.println("Requesting POS...");
    display.display();
    sendUDPMessage("GET_CURRENT_POS");

    // Wait for SET_POS from Controller
        ...
        if (packetSize) {
            ...
            if (response.startsWith("SET_POS")) {
                ...
                if (comma1 != -1 && comma2 != -1 && comma3 != -1) {
                    ...
                    Serial.println("Motor positions set: " + String(saved_position1) + "," + String(saved_position2) + "," + String(saved_position3) + "," + String(saved_position4));
                    // Send current positions back to Controller
                    sendCurrentMotorPositions();
                } else {
                    Serial.println("Error: Invalid SET_POS format");
                }
                ...
                int comma3 = response.indexOf(',', comma2 + 1);
                if (comma1 != -1 && comma2 != -1 && comma3 != -1) {
                    presetAPositions[0] = response.substring(12, comma1).toInt();
                    presetAPositions[1] = response.substring(comma1 + 1, comma2).toInt();
                    presetAPositions[2] = response.substring(comma2 + 1, comma3).toInt();
                    presetAPositions[3] = response.substring(comma3 + 1).toInt();
                    presetAReceived = true;
                    Serial.println("Preset A set: " + String(presetAPositions[0]) + "," + String(presetAPositions[1]) + "," + String(presetAPositions[2]) + "," + String(presetAPositions[3]));
                } else {
                    Serial.println("Error: Invalid SET_PRESET_A format");
                }
                ...
                int comma3 = response.indexOf(',', comma2 + 1);
                if (comma1 != -1 && comma2 != -1 && comma3 != -1) {
                    presetBPositions[0] = response.substring(12, comma1).toInt();
                    presetBPositions[1] = response.substring(comma1 + 1, comma2).toInt();
                    presetBPositions[2] = response.substring(comma2 + 1, comma3).toInt();
                    presetBPositions[3] = response.substring(comma3 + 1).toInt();
                    presetBReceived = true;
                    Serial.println("Preset B set: " + String(presetBPositions[0]) + "," + String(presetBPositions[1]) + "," + String(presetBPositions[2]) + "," + String(presetBPositions[3]));
                } else {
                    Serial.println("Error: Invalid SET_PRESET_B format");
                }
                ...
                stepper3.setMaxSpeed(msSpeed);
                stepper4.setMaxSpeed(msSpeed);
                Serial.println("msSpeed set to: " + String(msSpeed));
            }
            if (initialPositionsSet && presetAReceived && presetBReceived) break;
        }
    }
    ...
    if (!initialPositionsSet) {
        ...
        display.println("Retry POS (Attempt 2)...");
        display.display();
        sendUDPMessage("GET_CURRENT_POS");
        startTime = millis();
        while (millis() - startTime < CONTROLLER_TIMEOUT * 2) { // 20 seconds for second attempt
            int packetSize = Udp.parsePacket();
            if (packetSize) {
                int len = Udp.read(packetBuffer, 255);
                if (len > 0) packetBuffer[len] = 0;
    ...
    // Log if positions were not received
    if (!initialPositionsSet) {
        Serial.println("Failed to receive SET_POS after all attempts!");
    }
    ...
    } else if (initialPositionsSet && presetAReceived && presetBReceived) {
        display.println("All Data Loaded");
    } else {
        display.println("Missing Data");
    }
    display.display();
}
...
void loop() {
    ...
    if (currentMillis - previousMillis >= updateInterval) {
        // Only control motors if initial positions are set
        if (initialPositionsSet) {
            controlStepperMotors();
        }
        previousMillis = currentMillis;
    }
    // Periodically debug preset values
    if (currentMillis - lastPresetDebugTime >= PRESET_DEBUG_INTERVAL) {
        Serial.println("Current Preset A: " + String(presetAPositions[0]) + "," + String(presetAPositions[1]) + "," + String(presetAPositions[2]) + "," + String(presetAPositions[3]));
        Serial.println("Current Preset B: " + String(presetBPositions[0]) + "," + String(presetBPositions[1]) + "," + String(presetBPositions[2]) + "," + String(presetBPositions[3]));
        lastPresetDebugTime = currentMillis;
    }
    if (initialPositionsSet) {
        stepper1.runSpeed();
        stepper2.runSpeed();
    ...
    if (packetSize) {
        ...
        if (request.startsWith("SET_JOYSTICK")) {
            ...
            if (initialPositionsSet) {
                ...
                Serial.println("Ignoring SEND_CURRENT_POS: Initial positions not set yet.");
            }
        } else if (request.startsWith("SAVE_POS")) {
            if (initialPositionsSet) {
                saveMotorPositions();
            } else {
                Serial.println("Ignoring SAVE_POS: Initial positions not set yet.");
            }
        } else if (request.startsWith("RECALL_POS")) {
            if (initialPositionsSet) {
                recallMotorPositions();
            } else {
                Serial.println("Ignoring RECALL_POS: Initial positions not set yet.");
            }
            ...
            if (initialPositionsSet) {
                ...
                Serial.println("Ignoring RECALL_A: Initial positions not set yet.");
            }
        } else if (request.startsWith("RECALL_B")) {
            if (initialPositionsSet) {
                recallPresetB();
            } else {
                Serial.println("Ignoring RECALL_B: Initial positions not set yet.");
            }
...
void recallPresetA() {
    ...
    Serial.println("Moving to A: " + String(targetPositions[0]) + "," + String(targetPositions[1]) + "," + String(targetPositions[2]) + "," + String(targetPositions[3]));
...
void recallPresetB() {
    ...
    Serial.println("Moving to B: " + String(targetPositions[0]) + "," + String(targetPositions[1]) + "," + String(targetPositions[2]) + "," + String(targetPositions[3]));
    stepper1.moveTo(targetPositions[0]);
    stepper2.moveTo(targetPositions[1]);
    stepper3.moveTo(targetPositions[2]);
    stepper4.moveTo(targetPositions[3]);
...
float calcSync(float maxSpeed, float accel, float maxDistance, AccelStepper stepper) {
